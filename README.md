...
插件配置统一在 WebUI 维护。「全局配置」页包括 JX3API 基础地址、事件通道地址、TLS 校验、指令前缀，以及全局接口令牌、全局推送令牌和全局推栏标识。会话配置页维护群属凭据和失效池。运行日志页监控 WebSocket、事件投递、补推和赤兔链路。多条可用凭据都会轮询使用。启动时会迁移历史配置字段并清空迁移源。

全局配置页顺序为：JX3API 基础地址、全局接口令牌、JX3API 事件通道地址、全局推送令牌、全局推栏标识、TLS 校验和指令前缀。基础地址只用于 JX3API HTTP 请求；事件通道地址只用于主动推送 WebSocket；JX3BOX、UNUA 等第三方固定地址不受影响。

会话配置页按 AstrBot 管理员或认领人分组展示群聊会话；会话卡片显示凭据来源和状态，不把全局凭据写入群属池。页面加载时会校正历史认领身份。认领人会话可取消其整个认领资格，也可按真实 ID 维护该群的授权管理员列表。AstrBot 管理员拥有最高权限，未认领群聊的第一位成功绑定者会成为该群认领人；AstrBot 管理员重新绑定会接管该群。群属失效池令牌只读显示。主动推送开关仍由命令管理，可发送 `通知管理` 查看。只有群不存在或机器人已被移出群这类确定性失效才累计失败次数，适配器断连和平台抖动只记日志。页面还提供删除会话操作，删除后清空该会话数据并尽力通过 OneBot 自动退群。该页面仅供 AstrBot 后台主人使用，请勿暴露到公网。
...
- 先在目标群聊发送系统命令 `sid` 复制 UMO，再私聊机器人：
  ...
  - `推送令牌 <UMO> <推送令牌>`
  - `推栏 <UMO> <推栏标识>`
- 群属添加会真实检测：接口令牌和推送令牌剩余少于 100 次拒绝；推栏标识通过真实接口校验。全局添加只做去重和基础校验，不做真实检测。
- 接口令牌、推送令牌和推栏标识按类型独立回退。该类型有群属可用凭据时只用群属池；群属可用池为空时，只有管理员勾选该类型的「使用全局」才使用全局池。群属可用池恢复后自动取消勾选。
- 全局推栏标识默认对所有新老群启用；群属推栏标识存在时只用群属池。群属推栏标识删除或失效删除后，自动恢复使用全局推栏标识。
- 接口令牌和推送令牌明确无效、过期、次数不足或剩余为 0 时，立即进入失效池；推栏标识连续 3 次确认无效后才删除。
- 失效池令牌每天检测一次，恢复可用后移回；连续 30 天不可用则彻底删除。
- 查询接口令牌和查询推送令牌只返回当前实际来源的凭据：使用群属时返回群属；使用全局时返回全局。没有可用来源时返回配置提示。
...
| `物价 外观名称 [服务器]` | 外观行情记录；先按名称搜索，只搜到一条直接查询，多条时优先只列套装（没有套装则全部列出，不限制条数），回复序号继续查询；图片 | Token |
...
| `查询 服务器 角色` | 角色奇遇统计；按绝世、普通、宠物分栏，标题显示「已出-未出」数量，未触发的置后灰显并盖灰色「未出」章；图片 | Token |
| `奇遇珍卷 服务器 角色` | 奇遇珍卷横卷图；普通与宠物奇遇列卷，绝世奇遇合成大图，已出的显示彩色形象，未出的显示剪影，底部显示总进度与记录时间；图片 | Token |
...
| `查人 [服务器] 角色` | 按角色查询风纪记录，未写区服时用绑定服；文本 | Token |
...
3. 把 `templates/img`（含子目录 `sect`、`serendipity`、`奇遇珍卷`）中的图片读取为 base64 Data URL。
...
├── _conf_schema.json        # 兼容旧配置；运行配置由 WebUI 与本地库管理
├── requirements.txt         # Python 依赖
├── CHANGELOG.md             # 版本更新记录
├── LICENSE                  # GNU AGPL v3
├── data/
│   └── plugin_data.db       # 随包只读基础数据：心法别名与配装标识（配装 / 宏 查询用，运行期不写入）
├── core/
...
└── templates/
    ...
    ├── img/                 # 全部图片资源
        ...
        ├── sect/            # 门派与心法图标
        ├── serendipity/     # 奇遇图标
        └── 奇遇珍卷/         # 珍卷本地素材：卷轴静态件、普通奇遇、绝世奇遇、宠物奇遇
    └── font/                # 字体资源：沙盘行楷，以及奇遇珍卷内嵌的 Noto Sans SC 子集（含 OFL 许可）
tools/                       # 素材维护脚本：抓取/重建/校验奇遇珍卷素材
```
...
7. 如果新增运行时数据，使用参数化 SQL 并在初始化阶段创建表；敏感配置写入本地库，不得展示到公开配置页。
...
3. 接口令牌、推送令牌和推栏标识按类型独立回退。群属可用池为空时，必须由管理员勾选对应「使用全局」才使用全局池；群属可用池恢复后自动取消勾选。
4. WebUI 的「全局配置」页维护 JX3API 基础地址、全局接口令牌、事件通道地址、全局推送令牌、全局推栏标识、TLS 和指令前缀；会话配置页维护群属凭据和失效池；运行日志页监控 WebSocket、事件投递、补推和赤兔链路。
5. `APIClient` 默认校验 TLS 证书，可在全局配置页关闭。请求日志会脱敏 `token` 和 `ticket`。
