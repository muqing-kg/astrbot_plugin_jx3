...
| `jx3api_token` | `string` | 空 | 全局 JX3API 接口令牌。仅当会话勾选「使用全局接口令牌」时才会使用 |
...
插件页面只展示群聊会话，并按 AstrBot 管理员或认领人分组；可查看绑定区服、机器人开关、接口令牌、推送令牌、推栏标识、推送健康状态，以及两个独立的全局令牌开关。认领人会话可取消其整个认领资格，也可按真实 ID 维护该群的授权管理员列表。AstrBot 管理员拥有最高权限，未认领群聊的第一位成功绑定者会成为该群认领人。三类凭据在页面内完整展示，便于检查与修改。主动推送开关仍由命令管理，可发送 `通知管理` 查看。只有群不存在或机器人已被移出群这类确定性失效才累计失败次数，适配器断连和平台抖动只记日志。页面还提供删除会话操作，删除后清空该会话数据并尽力通过 OneBot 自动退群。该页面仅供 AstrBot 后台主人使用，请勿暴露到公网。
...
- 先在目标群聊发送系统命令 `sid` 复制 UMO，再私聊机器人：
  ...
  - `推送令牌 <UMO> <推送令牌>`
  - `推栏 <UMO> <推栏标识>`
- 任意用户都可以在私聊中为指定群聊写入接口令牌、推送令牌、推栏标识；写入前会通过 JX3API 或真实接口校验，校验失败不入库。
- 付费查询优先使用该会话自己的接口令牌；没有专属接口令牌且未勾选「使用全局接口令牌」时会提示去购买并配置。
...
3. 付费查询需要该会话接口令牌，或在插件页面勾选「使用全局接口令牌」。付费事件推送需要该群推送令牌，或勾选「使用全局推送令牌」后的全局推送令牌；未配置时只能使用免费事件档。推栏默认走全局，会话自定义优先。
4. 插件页面 `pages/dashboard` 以卡片列表展示已绑定区服的群聊，含区服、机器人开关、接口令牌/推送令牌/推栏配置状态和确定性推送失效次数；页面内可直接查看和编辑三类凭据原文，可取消认领资格，维护授权管理员列表，并可删除会话。OneBot 平台会尽力自动退群。
