...
插件页面只展示群聊会话，并按 AstrBot 管理员或认领人分组；可查看绑定区服、机器人开关、JX3API Token、推栏标识、推送健康状态，以及是否使用全局 Token。认领人会话可取消其整个认领资格，也可按真实 ID 维护该群的授权管理员列表。AstrBot 管理员拥有最高权限，绑定群聊时无需先私聊认领。Token、推栏标识在页面内完整展示，便于检查与修改。主动推送开关仍由命令管理，可发送 `通知管理` 查看。只有群不存在或机器人已被移出群这类确定性失效才累计失败次数，适配器断连和平台抖动只记日志。页面还提供删除会话操作，删除后清空该会话数据并尽力通过 OneBot 自动退群。该页面仅供 AstrBot 后台主人使用，请勿暴露到公网。
...
- 先在目标群聊发送系统命令 `sid` 复制 UMO，再私聊机器人：
  - `Token <UMO> <Token>`
  - `推栏 <UMO> <推栏标识>`
- 任意用户都可以在私聊中为指定群聊写入 Token、推栏标识；写入前会通过 JX3API 校验，Token 调用令牌统计接口，推栏标识真实调用一次，校验失败不入库。
- 付费指令优先使用该会话自己的 Token；没有专属 Token 且未勾选「使用全局 Token」时会提示去购买并配置。
- 推栏默认使用全局配置；某群聊自定义后只服务该群聊。
...
- 任何用户都可在私聊中为指定群聊写入 Token、推栏，但必须通过真实接口校验；Token 或推栏过期、次数不足时返回明确提醒。
...
| `武林争霸赛 [服务器] [阵营]` | 武林争霸赛，阵营默认恶人，支持恶人/恶人谷、浩气/浩气盟；图片 | Token |
...
- `session_config`：每个群聊的区服绑定、推送开关和密钥。历史私聊会话会在初始化时清理。
...
1. 每个群聊独立绑定一个区服；查询未写区服时使用绑定服，写了区服则以当场写的为准。QQ/微信私聊不再提供查询和主动推送，仅保留 `认领`、`Token`、`推栏` 配置入口。
2. 主动推送按会话开关和绑定区服发送，不再使用配置页里的 `kfts` / `xwts` / `smts` / `ctts`。
3. 付费指令需要该会话 Token，或在插件页面勾选「使用全局 Token」。推栏默认走全局，会话自定义优先。
4. 插件页面 `pages/dashboard` 以卡片列表展示已绑定区服的群聊，含区服、机器人开关、Token/推栏配置状态和确定性推送失效次数；页面内可直接查看和编辑 Token、推栏原文，可取消认领资格，维护授权管理员列表，并可删除会话。OneBot 平台会尽力自动退群。
