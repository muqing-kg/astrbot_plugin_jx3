...
`astrbot_plugin_jx3` 通过 JX3API 和 JX3BOX 查询《剑网3》游戏数据，并根据功能将结果发送为纯文本、图片、图文消息或两轮交互消息。插件同时提供本地避雷记录和开服、新闻、刷马、赤兔后台推送能力。
...
插件页面可查看每个群/私聊的绑定区服、推送开关、Token/推栏是否已配置，以及是否使用全局 Token。页面不回显密钥原文。该页面仅供 AstrBot 后台主人使用，请勿暴露到公网。
...
| `跨服名剑 [服务器] [模式]` | 跨服名剑榜，模式默认 `33`；图片 | Token |
| `武林争霸 [服务器] [阵营]` | 武林争霸榜，阵营默认浩气；图片 | Token |
| `捕快 [服务器]` | 捕快荣誉榜；图片 | Token |
| `浪客 [服务器]` | 江湖浪客榜；图片 | Token |
| `决斗 [服务器] [公开/私密]` | 决斗挑战榜，默认公开；图片 | Token |
| `试炼秒伤 赛季 层数` | 试炼之地指定层秒伤；图片 | Token |
| `试炼赛季` | 试炼赛季列表；图片 | Token |
...
| `物价 外观名称 [服务器]` | 外观价格记录，走 `/trade/item/records`；图片 | Token |
| `外观价格 外观名称 [服务器]` | `物价` 的同接口命令；图片 | Token |
...
| `名片记录 服务器 角色` | 远程取角色名片；图文 | Token |
| `名片预设 服务器 角色` | 推栏名片预设图；图文 | Token + Ticket |
...
| `配装 心法 [玩法]` | 推栏推荐配装；文本 | Token + Ticket |
| `急速 [技能CD]` | 按技能 CD 计算急速档位，默认 `1.5`；图片 | Token |
...
| 指令 | 作用 |
| --- | --- |
| `认领 剑网3机器人` | 认领插件管理员。之后仅认领人和 AstrBot 管理员可绑定区服、开关推送、配置密钥 |
| `绑定 区服名` | 为当前群/私聊绑定唯一区服 |
| `打开 新闻` / `关闭 新闻` | 打开或关闭当前会话的新闻推送 |
| `打开 开服` / `关闭 开服` | 打开或关闭当前会话的开服推送 |
| `打开 刷马` / `关闭 刷马` | 打开或关闭当前会话的刷马推送 |
| `打开 赤兔` / `关闭 赤兔` | 打开或关闭当前会话的赤兔推送 |
| `查询令牌` | 查询该群/该会话 Token 与全局 Token 的用量，不需要输入密钥 |
| `Token <UMO> <Token>` | 仅私聊。为指定会话保存 Token，不回显原文 |
| `推栏 <UMO> <推栏标识>` | 仅私聊。为指定会话保存自定义推栏 |

群聊中请使用系统命令 `/sid` 查询 UMO，不要在群里发送 Token 或推栏原文。A 群的推送只进 A 群，B 群只进 B 群。
...
4. 创建本地数据库、随包数据库、三个数据服务、会话绑定存储、避雷服务、推送调度器和消息构建器。
...
4. 插件页面 `pages/sessions` 只展示绑定状态和密钥是否已配置，不回显 Token/推栏原文。
