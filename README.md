...
| `jx3api_token` | `string` | 空 | 旧版全局接口令牌。启动时迁移到 WebUI 全局凭据池后清空 |
| `jx3api_ticket` | `string` | 空 | 旧版全局推栏标识。启动时迁移到 WebUI 全局凭据池后清空 |
...
插件页面先维护全局接口令牌池和全局推栏标识池，再按 AstrBot 管理员或认领人分组展示群聊会话；会话卡片只显示凭据来源和状态，不把全局凭据写入群属池。页面加载时会校正历史认领身份。认领人会话可取消其整个认领资格，也可按真实 ID 维护该群的授权管理员列表。AstrBot 管理员拥有最高权限，未认领群聊的第一位成功绑定者会成为该群认领人；AstrBot 管理员重新绑定会接管该群。群属移除池令牌只读显示。主动推送开关仍由命令管理，可发送 `通知管理` 查看。只有群不存在或机器人已被移出群这类确定性失效才累计失败次数，适配器断连和平台抖动只记日志。页面还提供删除会话操作，删除后清空该会话数据并尽力通过 OneBot 自动退群。该页面仅供 AstrBot 后台主人使用，请勿暴露到公网。
...
- 接口令牌和推栏标识按类型独立回退：该类型有群属可用凭据时只用群属池；没有时使用全局池。全局凭据在 WebUI 的全局凭据区维护，多条自动轮询。
- 群属接口令牌运行时连续确认 3 次失效或次数不足后进入移除池，并返回打码后的具体凭据；每日检查恢复条件，达到 100 次后自动移回可用池。
- 全局接口令牌确认失效或次数不足后只标记跳过并提醒，不移除；每日检查恢复后自动取消跳过。全局推栏标识确认失效后直接删除。
...
3. 付费查询按类型独立回退：接口令牌和推栏标识先群属池，再全局池。付费事件推送需要该群推送令牌，或勾选「使用全局推送令牌」后的全局推送令牌；未配置时只能使用免费事件档。
4. 插件页面 `pages/dashboard` 先维护全局接口令牌池和全局推栏标识池，再展示已绑定区服的群聊；群聊卡片含区服、机器人开关、凭据来源、移除池令牌、推送令牌和确定性推送失效次数。页面内可完整查看和编辑凭据，可取消认领资格，维护授权管理员列表，并可删除会话。OneBot 平台会尽力自动退群。
