...
| `matplotlib` | 当前依赖清单保留的绘图依赖；业务代码未直接导入 |
...
| 配置项 | 类型 | 默认值 | 说明 |
| --- | --- | --- | --- |
| `prefix.enable` | `bool` | `false` | 是否额外启用插件前缀。默认关闭，走 AstrBot 默认前缀 |
...
如果启用了插件前缀，也可以发送 `剑三 日常`。前缀与指令之间的空格可以省略。
...
- 每个群/私聊需先绑定区服。查询未写区服时使用该会话绑定服；当场写出区服则以写出的为准。
- 绑定、打开/关闭推送、配置 Token/推栏仅限认领的插件管理员或 AstrBot 管理员。
...
| `关隘首领` | 查询关隘首领状态；图片 | Token |
| `赤兔`、`本周赤兔` | 查询当日或本周赤兔记录；文本 | Token |
| `阵营事件 [阵营]` | 查询阵营奉献事件，固定最多 50 条；图片 | Token |
...
| `武林争霸 [服务器] [阵营]` | 武林争霸榜，阵营默认浩气；图片 | Token |
| `捕快荣誉榜 [服务器]` | 捕快荣誉榜；图片 | Token |
| `江湖浪客榜 [服务器]` | 江湖浪客榜；图片 | Token |
| `决斗挑战榜 [服务器] [公开/私密]` | 决斗挑战榜，默认公开；图片 | Token |
...
| 指令 | 说明与输出 | 凭据 |
| --- | --- | --- |
| `拍卖 服务器 [物品] [数量]` | 阵营拍卖记录，默认最多 50 条；图片 | Token |
...
| `全部名片 服务器 角色` | 历史名片，发送文字与多张图片 | Token |
...
项目级 `.gitignore` 只忽略 pytest 缓存与覆盖率产物，`tests/` 已纳入源码统一维护。语法检查和差异检查仍不能替代带真实数据的 AstrBot 消息、HTML 渲染、后台推送和外部接口联调；发布前应在具备有效凭据的实际环境中覆盖成功、空数据、超时及上游异常路径。
