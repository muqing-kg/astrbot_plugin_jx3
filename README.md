...
插件页面可查看每个群/私聊的绑定区服、机器人开关、JX3API Token、推栏标识，以及是否使用全局 Token；可查看认领人并取消其整个认领资格，也可编辑该群的授权管理员列表。Token、推栏标识在页面内完整展示，便于检查与修改。主动推送开关仍由命令管理，可发送 `通知管理` 查看。该页面仅供 AstrBot 后台主人使用，请勿暴露到公网。
...
- 群聊中不要发送 Token 或推栏原文。
- 先在目标群或私聊发送系统命令 `sid` 复制 UMO，再私聊机器人：
  - `Token <UMO> <Token>`
  - `推栏 <UMO> <推栏标识>`
- 任意用户都可以在私聊中为指定会话写入 Token、推栏标识；写入前会通过 JX3API 校验，Token 调用令牌统计接口，推栏标识真实调用一次，校验失败不入库。
- 付费指令优先使用该会话自己的 Token；没有专属 Token 且未勾选「使用全局 Token」时会提示去购买并配置。
- 推栏默认使用全局配置；某会话自定义后只服务该会话。
- Token 或推栏过期、次数不足时，命令会返回对应提醒，并要求更换后重试。
...
- `绑定`、`授权管理`、`删除管理` 仅限认领人或 AstrBot 管理员。
- `打开`、`关闭`、`查询令牌` 仅限 AstrBot 管理员、认领人或已授权管理员。
- 任何用户都可在私聊中为指定会话写入 Token、推栏，但必须通过真实接口校验；Token 或推栏过期、次数不足时返回明确提醒。
...
| `试炼之地排行 服务器 心法` | 试炼之地排行；图片 | Token |
| `跨服名剑榜 [服务器] [模式]` | 跨服名剑榜，模式默认 `33`；图片 | Token |
| `武林争霸赛 [服务器] [阵营]` | 武林争霸赛，阵营默认浩气，支持恶人/恶人谷、浩气/浩气盟；图片 | Token |
...
- `styles/pages/*.css`：仅给无法复用公共组件的复杂页面补充独立布局，当前 46 个页面中有 18 个需要专属 CSS。
...
4. 插件页面 `pages/dashboard` 以卡片列表展示已绑定区服的群/私聊，含区服、机器人开关、Token/推栏配置状态；页面内可直接查看和编辑 Token、推栏原文，可取消认领资格，并维护授权管理员列表。
