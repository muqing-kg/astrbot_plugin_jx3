...
插件配置统一在 WebUI 的「全局配置」页维护，包括 JX3API 基础地址、事件通道地址、TLS 校验、指令前缀，以及全局接口令牌、全局推送令牌和全局推栏标识。多条可用凭据都会轮询使用。启动时会迁移历史配置字段并清空迁移源。
...
4. WebUI 的「全局配置」页维护 JX3API 基础地址、全局接口令牌、事件通道地址、全局推送令牌、全局推栏标识、TLS 和指令前缀；会话配置页维护群属凭据和失效池。
