...
class Jx3ApiPlugin(Star):
    ...
    @jx3.command("名片")
    async def jx3_jueshemingpian(self, event: AstrMessageEvent,server: str = "梦江南", name: str = "[已清除]"):
        """剑三 名片 服务器 角色"""
        try:
    ...
    @jx3.command("烟花")
    async def jx3_yanhuachaxun(self, event: AstrMessageEvent,server: str = "梦江南", name: str = "[已清除]"):
        """剑三 烟花 服务器 角色"""
        try:
