...
@register("astrbot_plugin_jx3", 
          ...
          "1.0.0",
          "https://github.com/[已清除]/astrbot_plugin_jx3api.git"
)
class Jx3ApiPlugin(Star):
    ...
    @jx3.command("名片")
    async def jx3_jueshemingpian(self, event: AstrMessageEvent, name: str = "[已清除]", server: str = None):
        """剑三 名片 角色 服务器"""
        try:
    ...
    @jx3.command("烟花")
    async def jx3_yanhuachaxun(self, event: AstrMessageEvent,name: str = "[已清除]", server: str = None):
        """剑三 烟花 角色 服务器"""
        try:
    ...
    @jx3.command("战绩")
    async def jx3_zhanji(self, event: AstrMessageEvent,name: str = "[已清除]", server: str = None, mode:str = "33"):
        """剑三 战绩 角色 服务器 类型"""
        try:
    ...
    @jx3.command("奇遇")
    async def jx3_qiyu(self, event: AstrMessageEvent,name: str = "[已清除]", server: str = None):
        """剑三 奇遇 角色名称 服务器"""
        try:
