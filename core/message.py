...
class MessageBuilder:
    ...
    async def jx3_yanhuachaxun(self, event: AstrMessageEvent,name: str = "[已清除]", server: str = ""):
        """剑三 烟花 角色 服务器"""
    ...
    async def jx3_juesheqiyu(self, event: AstrMessageEvent,name: str = "[已清除]", server: str = ""):
        """剑三 奇遇 角色名称 服务器"""
        return await self.T2I_image_msg(event, lambda: self.jx3api.juesheqiyu(name, self.serverdefault(server)))
    

    async def jx3_weizuoqiyu(self, event: AstrMessageEvent,name: str = "[已清除]", server: str = ""):
        """剑三 未做奇遇 角色名称 服务器"""
    ...
    async def jx3_zhanji(self, event: AstrMessageEvent,name: str = "[已清除]", server: str = "", mode:str = "33"):
        """剑三 战绩 角色 服务器 类型"""
    ...
    async def jx3_jueshemingpian(self, event: AstrMessageEvent, name: str = "[已清除]", server: str = ""):
        """剑三 名片 角色 服务器"""
        return await self.plain_chain(event, lambda: self.jx3api.jueshemingpian(self.serverdefault(server), name)) 


    async def jx3_jingnai(self, event: AstrMessageEvent, name: str = "[已清除]", server: str = ""):
        """剑三 精耐 角色 服务器"""
        return await self.T2I_image_msg(event, lambda: self.jx3api.jingnai(name, self.serverdefault(server)))


    async def jx3_shuoyoumingpian(self, event: AstrMessageEvent, name: str = "[已清除]", server: str = ""):
        """剑三 所有名片 角色 服务器"""
